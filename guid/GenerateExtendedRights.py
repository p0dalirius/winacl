...
f = open("./ExtendedRights.go", "w")
f.write("package guid\n\n")

f.write("const (\n")
for name, guid in extendedrights.items():
    f.write('\t%s = "%s"\n' % ("EXTENDED_RIGHT_"+name.upper().replace('-','_'), guid))
f.write(")\n\n\n")

f.write("var ExtendedRightToGUID = map[string]string{\n")
for name, guid in extendedrights.items():
    f.write('\t"%s": %s,\n' % ("EXTENDED_RIGHT_"+name.upper().replace('-','_'), "EXTENDED_RIGHT_"+name.upper().replace('-','_')))
f.write("}\n\n\n")

f.write("var GUIDToExtendedRight = map[string]string{\n")
for name, guid in extendedrights.items():
    f.write('\t%s: "%s",\n' % ("EXTENDED_RIGHT_"+name.upper().replace('-','_'), "EXTENDED_RIGHT_"+name.upper().replace('-','_')))
f.write("}\n\n\n")
