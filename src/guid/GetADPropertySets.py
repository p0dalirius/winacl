...
if __name__ == "__main__":
    ...
    if options.auth_username is not None:
        ...
        invertedConstPropertySets = {v:k for k,v in constPropertySets.items()}

        f = open("./PropertySets.go", "w")
        f.write("package guid\n\n")

        f.write("const (\n")
        for name, guid in constPropertySets.items():
            f.write('\t%s = "%s"\n' % (name.upper(), guid))
        f.write(")\n\n\n")

        f.write("var PropertySetToGUID = map[string]string{\n")
        for name, guid in constPropertySets.items():
            f.write('\t"%s": %s,\n' % (name.upper(), name.upper()))
        f.write("}\n\n\n")

        f.write("var GUIDToPropertySet = map[string]string{\n")
        for name, guid in constPropertySets.items():
            f.write('\t%s: "%s",\n' % (name.upper(), name.upper()))
        f.write("}\n\n\n")

        propertySets = {}
        for key, value in ls.schemaIDGUID.items():
            if "attributeSecurityGUID" in value.keys():
                attributeSecurityGUID = GUID.fromRawBytes(value["attributeSecurityGUID"]).toFormatD()

                if attributeSecurityGUID not in propertySets.keys():
                    propertySets[attributeSecurityGUID] = []
                propertySets[attributeSecurityGUID].append(value["name"].lower())



        f.write("var PropertySets = map[string][]string{\n")
        for key, value in sorted(propertySets.items(), key=lambda x:x[0]):
            guidname = key
            if key in invertedConstPropertySets.keys():
                guidname = invertedConstPropertySets[key]

            f.write(
                '\t"%s": {%s},\n' % (
                    guidname, 
                    ', '.join(['"%s"'%e for e in sorted(value)])
                )
            )
        f.write("}\n\n\n")
